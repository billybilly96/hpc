...
void init_map( cell_t *ghost, cell_t *current, int width, int newWidth, int R ) {
		int i,j;
		#pragma omp parallel for default(none) private(i,j) collapse(2) shared(width,newWidth,R,ghost,current) schedule(static,width*2)
		for (i=0; i<width; i++) {
				for (j=0; j<width; j++) {
...
void fill_ghost_cells( cell_t *ghost, int width, int newWidth, int R ) {
		int i,j;
		#pragma omp parallel default(none) private(i,j) shared(width,newWidth,R,ghost)
		#pragma omp for collapse(2) schedule(static,R*2)
		for (i=0; i<R; i++) {
				for (j=0; j<width; j++) {
		...
		#pragma omp for collapse(2) schedule(static,(width+R)*2)
...
void processGeneration( int newWidth, cell_t *ghost, int B1, int B2, int D1, int D2, int R, cell_t *temp ) {
        ...
        /*#pragma omp parallel for default(none) private(i,j,neighbors) collapse(2) shared(newWidth,R,ghost,temp,B1,B2,D1,D2) schedule(static,newWidth)*/
        #pragma omp for collapse(2) schedule(static,newWidth*2)     
        ...
        /*#pragma omp parallel for default(none) private(i,j) collapse(2) shared(newWidth,ghost,temp) schedule(static,newWidth)*/
        #pragma omp for collapse(2) schedule(static,newWidth*2) 
